...
class PressureSweepWindow(QMainWindow):
    pressure_sweep_sig = Signal(str, str, str)  # span, rate, direction
    ...
    def create_gui(self) -> None:
        ...
        # Create the widgets
        self.span_label = QLabel('Pressure Span (mBar)')
        self.span_entry = QLineEdit('400')
        self.span_entry.setValidator(entry_validator)

        self.rate_label = QLabel('Change Rate (mBar/sec)')
        ...
        rb_group_layout = QHBoxLayout()
        rb_group_layout.addWidget(self.h2l_rb)
        rb_group_layout.addWidget(self.l2h_rb)
